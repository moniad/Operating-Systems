...
int main(int argc, char **argv){
    ...
    while (cycles-- != 0){ // cause it can be equal to -1 if not specified
        ...
        while(flag){
            ...
            printf("loader: mam taśmę\n");
            if(semop(sem_belt_weight_id, &sem_belt_weight_op, 1) == 0) { // OK
                ...
                printf("getpid() = %d, cycles = %d\n", getpid(), cycles);
                give_back_belt_op();
                continue;
            }
            else {
                printf("ELSE getpid() = %d, cycles = %d\n", getpid(), cycles);
                die_errno("semop belt_weight");
            }
            if(semop(sem_belt_operation_id, &sem_belt_operation_op_give, 1) < 0) die_errno("semop belt_op_take");
            sleep(2);
        }
        printf("loader: oddaję taśmę\n");
