...
void create_and_init_shm(){
    ...
    if((belt_id = shmget(belt_key, (size_t)SHM_SIZE(MAX_BELT_LENGTH), IPC_CREAT | IPC_EXCL | 0600)) < 0) die_errno("shmget()");
...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        printf("trucker: oddaję taśme\n");
        if(semop(sem_belt_operation_id, &sem_belt_operation_op_give, 1) < 0) die_errno("semop belt_op_give");
        sleep(2);
    }
